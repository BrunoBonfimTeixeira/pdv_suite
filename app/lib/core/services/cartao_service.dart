...
class CartaoService {
  static Future<List<CartaoOperadora>> listar() async {
    try {
      final res = await ApiClient.dio.get('/cartoes');
      if (res.data is List) {
        return (res.data as List)
            .whereType<Map>()
            .map((m) => CartaoOperadora.fromJson(Map<String, dynamic>.from(m)))
            .toList();
      }
      return [];
  ...
  static Future<int> criar(Map<String, dynamic> data) async {
    try {
      final res = await ApiClient.dio.post('/cartoes', data: data);
      final body = res.data;
  ...
  static Future<void> atualizar(int id, Map<String, dynamic> data) async {
    try {
      await ApiClient.dio.patch('/cartoes/$id', data: data);
    } on DioException catch (e) {
      final msg = e.response?.data?.toString() ?? e.message ?? 'Erro';
      throw Exception(msg);
    }
  ...
  static Future<void> remover(int id) async {
    try {
      await ApiClient.dio.delete('/cartoes/$id');
    } on DioException catch (e) {
      final msg = e.response?.data?.toString() ?? e.message ?? 'Erro';
      throw Exception(msg);
    }

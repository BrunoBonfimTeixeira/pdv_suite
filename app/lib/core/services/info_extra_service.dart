...
class InfoExtraService {
  static Future<List<InfoExtra>> listarPorProduto(int produtoId) async {
    try {
      final res =
          await ApiClient.dio.get('/info-extras/produto/$produtoId');
      if (res.data is List) {
        return (res.data as List)
            .whereType<Map>()
            .map((m) => InfoExtra.fromJson(Map<String, dynamic>.from(m)))
            .toList();
      }
      return [];
  ...
  static Future<int> criar(Map<String, dynamic> data) async {
    try {
      final res = await ApiClient.dio.post('/info-extras', data: data);
      final body = res.data;
  ...
  static Future<void> atualizar(int id, Map<String, dynamic> data) async {
    try {
      await ApiClient.dio.patch('/info-extras/$id', data: data);
    } on DioException catch (e) {
      final msg = e.response?.data?.toString() ?? e.message ?? 'Erro';
      throw Exception(msg);
    }
  ...
  static Future<void> remover(int id) async {
    try {
      await ApiClient.dio.delete('/info-extras/$id');
    } on DioException catch (e) {
      final msg = e.response?.data?.toString() ?? e.message ?? 'Erro';
      throw Exception(msg);
    }
